...
use engine_wgpu_wrapper::{EngineType, WgpuWrapper, SPHERES};
...
impl Model {
    pub fn new() -> Self {
        Self {
            renderer: WgpuWrapper::new(EngineType::Raytracer, 500, 500, FOV).expect("Renderer initialization failed")
        }
    }

    pub fn generate_render_output(&mut self) -> RenderOutput {
        self.renderer.render(RenderCommand {
            fov: Some(FOV),
